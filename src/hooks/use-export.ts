...
export function useExport() {
  const exportToPDF = useCallback(async (inventory: Inventory) => {
    try {
      ...
      pdf.text(inventory.name, pageWidth / 2, yPosition, {
        align: "center",
      });
      yPosition += 20;
      ...
      pdf.setFont("helvetica", "normal");
      const totalQuantity = inventory.products.reduce(
      ...
      pdf.text(
        `Data da Contagem: ${new Date(inventory.createdAt).toLocaleDateString(
          "pt-BR"
        )}`,
        margin,
        yPosition
      ...
      // Salva o PDF
      const fileName = `contagem-${inventory.name
        .toLowerCase()
  ...
  const exportToCSV = useCallback((inventory: Inventory) => {
    try {
      const totalQuantity = inventory.products.reduce(
        (sum, item) => sum + item.quantity,
        0
      ...
      csvContent += `Nome da Contagem,${inventory.name}\n`;
      csvContent += `Data,${new Date(inventory.createdAt).toLocaleDateString(
        "pt-BR"
      )}\n`;
      csvContent += `Total de Produtos,${inventory.products.length}\n`;
